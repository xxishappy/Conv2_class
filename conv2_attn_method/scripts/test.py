...
OUTPUT_DIR = "/data/users/dyx/Myproject/FetalPlaneClass/output/conv2_attn_method/polar/none/test"
...
CHECKPOINT_PATH="/data/users/dyx/Myproject/FetalPlaneClass/output/conv2_attn_method/polar/none/f1_92.84458-epoch_139.pth"
NUM_CLASSES = 22
...
PRIOR_TYPE="none" #rect,gaussian,polar
PRIOR_ALPHA=0.5
POLAR_POLE='up' #up,down,left,right,random
...
def evaluate(model,loader,device):
    ...
    for batch_index , (images,masks,labels,image_paths) in enumerate(loader):
        ...
        with autocast_context(device):
            features = model.forward_features(images,mask=masks)
        ...
        for item_index, (true_label, pred_label) in enumerate(zip(labels_np, preds_np)):
            if int(true_label) != int(pred_label):
                misclassified.append({
                    "dataset_index": batch_index * BATCH_SIZE + item_index,
...
def main():
    ...
    test_set = FetalPlaneDataset(excel_path=EXCEL_PATH, split="test",return_path=True,return_mask=True)
    ...
    if PRIOR_TYPE=="polar":
        model_kwargs["pole"]=POLAR_POLE

    model = getattr(convnextv2, MODEL)(**model_kwargs).to(device)
