...
PRIOR_ALPHA=0.5
POLAR_POLE='random' #up,down,left,right,random
...
def train_one_epoch(model, loader, criterion, optimizer, scaler, epoch):
    ...
    for step, (images, masks,labels) in enumerate(loader):
        ...
        with autocast(dtype=amp_dtype, enabled=amp_enabled):
            logits = model(images,mask=masks)
...
def main():
    ...
    train_set = FetalPlaneDataset(excel_path=EXCEL_PATH, split="train",return_mask=True)
    val_set = FetalPlaneDataset(excel_path=EXCEL_PATH, split="valid",return_mask=True)
    ...
    model=model_fn(**model_kwargs).cuda()
